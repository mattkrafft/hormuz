...
function drawLabels(){ctx.save();ctx.textAlign="center";ctx.fillStyle="#abd850aa";ctx.font="18px Share Tech Mono";ctx.fillText("I R A N",W*.58,H*.16);ctx.font="14px Share Tech Mono";ctx.fillText("Q A T A R",W*.105,H*.20);ctx.fillText("U A E",W*.38,H*.76);ctx.fillText("O M A N",W*.76,H*.84);ctx.fillStyle="#8bc46c88";ctx.font="13px Share Tech Mono";ctx.fillText("P E R S I A N   G U L F",W*.17,H*.56);ctx.fillText("G U L F   O F   O M A N",W*.85,H*.55);ctx.fillStyle="#d8c849";ctx.font="11px Share Tech Mono";ctx.fillText("Dubai",W*.45,H*.75);ctx.fillText("Fujairah",W*.72,H*.72);ctx.save();ctx.translate(W*.48,H*.49);ctx.rotate(-.13);ctx.fillStyle="#9ee75b";ctx.fillText("EASTBOUND SHIPPING LANE",0,0);ctx.restore();ctx.restore()}
